    ...
    return (
        <aside className={`${styles.sidePanel} ${show ? styles.sidePanelOpen : styles.sidePanelClosed}`}>
            <div className={`${styles.sidePanel__content} ${active === 0 && styles.contentActive}`}>
                ...
                >Synopsis</span>
                <div>
                    <h2>Q1 Quantum Synth</h2>
                    <p>Quantum sound design is achieved by rotating and measuring single quantum bits.</p>
                    <p>MouseY for angle of inclination (θ).</p>
                    <p>MouseX for angle of azimuth (φ).</p>
                    <p>Shift + MouseX for sphere phase (λ).</p>
                    <p>F key into full screen mode.</p>
                    <p>Hold down Shift and click a preset key (1-10) to save the current modulation state.</p>
                </div>
            </div>
            {mode === 'advanced' &&
                <>
                            ...
                            <div className={styles.scrollableContainer}>
                                {selectedGates.map((selectedGate, index) => (
                                    <div key={index} className={styles.selectedGateBox}>
                                        ...
                                        <button className={styles.deleteButton} onClick={() => handleDeleteGate(index)}>Delete</button>
                                    </div>
                                ))}
                            </div>
                        </div>
